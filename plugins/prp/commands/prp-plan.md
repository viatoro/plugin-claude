...
5. **Report selection to user:**
   ...
   Note: Phase {X} can also run in parallel (coordinate commits to avoid conflicts).
...
{If parallel phases available:}
**Parallel Opportunity**: Phase {X} can run concurrently on main (coordinate commits).
To start parallel work: Implement in small commits, pull frequently, use feature flags for incomplete work.
